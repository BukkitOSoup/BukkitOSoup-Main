	...
	public void injectPlayer(Player... player){
		...
		for(Player p : player){
			...
			for (Entry<String, Boolean> perm : attachments.get(p.getUniqueId()).getPermissions().entrySet()) {
				if (getPerms(p).contains(perm.getKey())) attachments.get(p.getUniqueId()).setPermission(perm.getKey(), true);
				else attachments.get(p.getUniqueId()).setPermission(perm.getKey(), false);
